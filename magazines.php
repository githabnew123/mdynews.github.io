...
<body>
    ...
    <h1 class="news-heading"  style="margin-top: 120px;">Magazines</h1>
<?php
  $data = get_magzines();
  $limit = 4;
  $x = sizeof($data);
  ...
  $y = 1;
    ...
    if (isset($_GET['limit'])) {
        ...
        $limit = post_target($page_target,'Local');
        $b = ($_GET['limit']*4);
    }else{
        $_GET['limit']=0;
    }
?>
  ...
  <p class="page-switch-paragraph">
      <a href="magazines.php?limit=<?php echo ($_GET['limit']-1); ?>"><button class="btn btn-outline-dark font-size" type="button" name="button"> < </button></a>
  ...
  <section id="news">
    <?php if ($limit>4 || $limit ==4 || $b==0): ?>
     <?php for($i=0;$i<4;$i++): ?>
       ...
       <a href="post_info.php?post_target=<?php echo $data[$i]['id']; ?>&post_type=Magazine" class="btn btn-info"><span class="w-25">See More</span></a>
       </div>
     </div>
...
<?php if($limit<4 && $limit>0 && $b>0 && $b<sizeof($data)): ?>
    <?php for($i=0;$i<$limit;$i++): ?>
       ...
       <a href="post_info.php?post_target=<?php echo $data[$b-1]['id']; ?>&post_type=Magazine" class="btn btn-info"><span class="w-25">See More</span></a>
       </div>
     </div>
